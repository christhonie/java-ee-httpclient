...
public class SendFileServlet extends HttpServlet {
        ...
        private static final Logger LOGGER = Logger.getLogger(SendFileServlet.class.getName());

        private static final String TARGET_URL = "https://daa-dev.development.creditguarantee.co.za/api/submissions";
        ...
        protected void doGet(HttpServletRequest req, HttpServletResponse resp)
                ...
                try (InputStream fileStream = ctx.getResourceAsStream("/WEB-INF/" + FILE_NAME)) {
                        ...
                        try {
                                sslContext = SSLContexts.custom()
                                                .loadTrustMaterial(null,
                                                                (X509Certificate[] chain, String authType) -> true) // trust
                                                                                                                    // all
                                                .build();
                        } catch (NoSuchAlgorithmException | KeyStoreException | KeyManagementException e) {
                                LOGGER.log(Level.SEVERE, "SendFileServlet: failed to build permissive SSLContext", e);
                                resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                                resp.setContentType("text/plain; charset=UTF-8");
                                resp.getWriter().println("Failed to set up SSL context: " + e.getMessage());
                                return;
                        }
                        ...
                        try (CloseableHttpClient client = HttpClients.custom()
                                ...
                                try (CloseableHttpResponse upstreamResponse = client.execute(httpPost)) {
                                        ...
                                        String body = upstreamResponse.getEntity() != null
                                                        ? EntityUtils.toString(upstreamResponse.getEntity(),
                                                                        StandardCharsets.UTF_8)
                                                        : "";

                                        LOGGER.info("SendFileServlet: response received from API, status = "
                                                        + statusCode);
                                        if (statusCode >= 400) {
                                                LOGGER.warning("SendFileServlet: API returned error status. Body: "
                                                                + body);
                                        } else {
